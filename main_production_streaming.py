...
indexPath_2000=r"Indices/content/index"
...
def get_response(prompt,message_history):
    ...
    response = chat_engine.stream_chat(prompt.lower())
    ...
    if feedback.text==str(0): #feedback.text
        ...
        df = pd.read_csv(f'logs/June20_index.csv')
        ...
        df.to_csv(f'logs/June20_index.csv', index=False)
        print("sucess")
        ...
        s3_resource.Object(bucket, 'June20_index.csv').put(Body=csv_buffer.getvalue()) 
...
def get_web_answer(question,context_from_rag):
    client = openai.OpenAI(api_key="pplx-api-key", base_url="https://api.perplexity.ai")
    prompt = (
        ...
        "Rely heavily on information recieved from the web. Be verbose.\n"
        )
    messages = [{"role":"user","content": prompt}]
    response = st.write_stream(client.chat.completions.create(model="llama-3-sonar-large-32k-online",messages=messages,stream=True))
    return response
